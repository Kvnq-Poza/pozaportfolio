...
const AVAILABLE_PATHS = ["/", "/about", "/projects", "/resume", "/contact", "/secret"] as const
...
export function Terminal({ onClose, onEasterEgg, history, onAddHistory, onNavigate }: TerminalProps) {
  ...
  useEffect(() => {
    const persistentHistory = history.flatMap((entry) => [`$ ${entry.command}`, ...entry.output, ""])
  ...
  const executeCommand = useCallback(
    async (commandStr: string) => {
      ...
      switch (cmd) {
        ...
        case "whoami":
          showOutput(commandStr, ["poza", "Full Stack Developer", "Available for hire"])
          return

        case "ls":
          if (currentPath === "/") {
            showOutput(commandStr, ["about/", "projects/", "resume/", "contact/", "secret/", "README.md"])
          } else {
        ...
        case "cd": {
          ...
          if (targetPath.startsWith("/")) {
            if ((AVAILABLE_PATHS as readonly string[]).includes(targetPath)) {
              ...
              showOutput(commandStr, [`cd: ${targetPath}: No such file or directory`])
              return
            }
          }
        ...
        case "cat": {
          ...
          if (filename === "readme.md") {
            try {
              ...
              showOutput(commandStr, ["Error loading README.md", String(err?.message || err)])
            }
          } else if (filename) {
            showOutput(commandStr, [`cat: ${filename}: No such file or directory`])
          } else {
        ...
        default:
          showOutput(commandStr, [`Command not found: ${cmd}. Type 'help' for available commands.`])
          return
      }
    },
      ...
      >
        ...
        <div className="flex items-center justify-between p-3 bg-gray-800 border-b border-gray-700">
          <div className="flex items-center space-x-2">
            ...
            <TerminalIcon className="w-4 h-4 text-gray-400 ml-2" aria-hidden="true" />
            <span className="text-gray-300 text-sm">Terminal</span>
          </div>
          <Button variant="ghost" size="sm" onClick={onClose} className="text-gray-400 hover:text-white" aria-label="Close terminal">
            <X className="w-4 h-4" />
          </Button>
        </div>
        ...
        <div className="flex flex-col h-[calc(100%-60px)]">
          <div ref={terminalRef} className="flex-1 p-4 overflow-y-auto font-mono text-sm">
            {displayHistory.map((line, index) => (
              <div key={index} className="text-green-400 whitespace-pre-wrap mb-1">
                {line}
              </div>
            ))}
            ...
            <AnimatePresence>
              {showSuggestions && suggestions.length > 0 && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="mt-2 p-2 bg-gray-800 rounded border border-gray-600">
